# ------------------------------------------------------------------------
# AIMET Quantization Training for NAFNet
# Supports a8w16 (8-bit activation, 16-bit weight) quantization
# ------------------------------------------------------------------------
import argparse
import datetime
import logging
...
def create_quantsim_model(model, dummy_input, config_file=None, 
                          default_output_bw=8, default_param_bw=16):
    """创建 AIMET QuantSim 模型"""
...
def parse_options(is_train=True):
    ...
    parser.add_argument('--param_bw', type=int, default=16,
                       help='Parameter bitwidth (default: 16)')
...
def init_loggers(opt):
    ...
    log_file = osp.join(opt['path']['log'],
                        f"train_aimet_{opt['name']}_{get_time_str()}.log")
    logger = get_root_logger(
...
def main():
    ...
    # 创建 AIMET QuantSim 模型
    logger.info("Creating AIMET QuantSim model...")
    ...
    # QAT 训练循环
    logger.info(f'Start QAT training from epoch: 0, iter: 0')
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if current_iter % opt['logger']['save_checkpoint_freq'] == 0:
                logger.info('Saving quantized models and training states.')
                model.save(epoch, current_iter)
                
                # 导出量化编码
                encoding_path = osp.join(opt['path']['models'], 
                                        f'quantsim_encodings_{current_iter}.json')
                quantsim.export_encodings(encoding_path)
                logger.info(f'Quantization encodings saved to {encoding_path}')
    ...
    logger.info(f'End of QAT training. Time consumed: {consumed_time}')
    logger.info('Save the latest quantized model.')
    model.save(epoch=-1, current_iter=-1)
    
    # 导出最终量化编码
    final_encoding_path = osp.join(opt['path']['models'], 'quantsim_encodings_final.json')
    quantsim.export_encodings(final_encoding_path)
    logger.info(f'Final quantization encodings saved to {final_encoding_path}')
