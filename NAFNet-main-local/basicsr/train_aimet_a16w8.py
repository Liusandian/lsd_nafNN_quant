# ------------------------------------------------------------------------
# AIMET Quantization Training for NAFNet (A16W8)
...
class WeightAugmentation:
    """权重扰动增强，专门针对 8-bit 权重量化"""
    
    def __init__(self, noise_std=0.001, aug_prob=0.3):
...
class AdvancedBLCNoiseAugmentation:
    """增强版 BLC 噪声增强，针对 a16w8 特点优化"""
    
    def __init__(self, blc_std=2.5, fpn_row_std=1.5, fpn_col_std=1.5, aug_prob=0.5):
...
class MixedPrecisionAugmentation:
    ...
    def __call__(self, lq, gt):
        ...
        # 随机选择增强类型
        aug_type = random.choice(['brightness', 'contrast', 'gamma'])
        ...
        if aug_type == 'brightness':
            ...
            lq = torch.pow(lq, gamma)
            gt = torch.pow(gt, gamma)
...
def create_quantsim_model(model, dummy_input, config_file=None, 
                          default_output_bw=16, default_param_bw=8):
    """创建 AIMET QuantSim 模型 (a16w8)"""
...
def parse_options(is_train=True):
    ...
    parser.add_argument('--activation_bw', type=int, default=16,
                       help='Activation bitwidth (default: 16)')
    ...
    # A16W8 特定参数
    parser.add_argument('--weight_aug', action='store_true',
                       help='Enable weight augmentation for better 8-bit weight quantization')
    parser.add_argument('--mixed_precision_aug', action='store_true',
                       help='Enable mixed precision augmentation')
    ...
    opt['aimet'] = {
        ...
        'mixed_precision_aug': args.mixed_precision_aug
    }
...
def init_loggers(opt):
    ...
    log_file = osp.join(opt['path']['log'],
                        f"train_aimet_a16w8_{opt['name']}_{get_time_str()}.log")
    logger = get_root_logger(
...
def main():
    ...
    # 创建 AIMET QuantSim 模型 (a16w8)
    logger.info("Creating AIMET QuantSim model for A16W8...")
    ...
    # 初始化数据增强
    blc_aug = AdvancedBLCNoiseAugmentation(aug_prob=0.5)
    mixed_aug = MixedPrecisionAugmentation(aug_prob=0.4) if opt['aimet']['mixed_precision_aug'] else None
    weight_aug = WeightAugmentation(aug_prob=0.3) if opt['aimet']['weight_aug'] else None
    
    logger.info(f"Data augmentation enabled:")
    logger.info(f"  - BLC Noise: Yes")
    logger.info(f"  - Mixed Precision: {opt['aimet']['mixed_precision_aug']}")
    ...
    # QAT 训练循环
    logger.info(f'Start A16W8 QAT training from epoch: 0, iter: 0')
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            # 混合精度增强
            if mixed_aug is not None and random.random() < 0.4:
                lq, gt = mixed_aug(lq, gt)
            
            train_data['lq'] = lq
            train_data['gt'] = gt
            
            # 权重增强（每 N 个 iter）
            if weight_aug is not None and current_iter % 5 == 0:
            ...
            if current_iter % opt['logger']['save_checkpoint_freq'] == 0:
                logger.info('Saving A16W8 quantized models and training states.')
                model.save(epoch, current_iter)
                
                # 导出量化编码
                encoding_path = osp.join(opt['path']['models'], 
                                        f'quantsim_a16w8_encodings_{current_iter}.json')
                quantsim.export_encodings(encoding_path)
                logger.info(f'A16W8 quantization encodings saved to {encoding_path}')
    ...
    logger.info(f'End of A16W8 QAT training. Time consumed: {consumed_time}')
    logger.info('Save the latest A16W8 quantized model.')
    model.save(epoch=-1, current_iter=-1)
    
    # 导出最终量化编码
    final_encoding_path = osp.join(opt['path']['models'], 'quantsim_a16w8_encodings_final.json')
    quantsim.export_encodings(final_encoding_path)
    logger.info(f'Final A16W8 quantization encodings saved to {final_encoding_path}')
